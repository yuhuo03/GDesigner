...
SVAMP_DEFAULT_PATH = "datasets/SVAMP/SVAMP.json"
...
class SVAMPDataset:
    def __init__(
        ...
        data_path: Union[str, Path] = SVAMP_DEFAULT_PATH,
        seed: int = 888,
        train_limit: int = 40,
    ) -> None:
    ...
    def _load_split(self) -> List[Dict[str, Any]]:
        records = svamp_data_process(load_svamp(self.data_path))
        if self._split == "full":
            ...
            raise ValueError(f"Unsupported SVAMP split: {self._split}")

        print(f"[SVAMPDataset] Loaded {len(selected_records)} samples from {self.data_path} split {self._split}")
...
def load_svamp(path: Union[str, Path] = SVAMP_DEFAULT_PATH) -> list:
    data_path = Path(path)
    if not data_path.exists():
        raise FileNotFoundError(f"Missing SVAMP data file at {data_path}.")
    with open(data_path, encoding="utf-8") as file:
        data = json.load(file)
    return data


def svamp_data_process(dataset: list) -> List[Dict[str, Any]]:
    records = []
    for item in dataset:
        body = item["Body"].strip()
        question = item["Question"].strip()
        full_question = f"{body} {question}".strip()
        answer = str(item["Answer"]) if item.get("Answer") not in (None, "") else ""
        records.append({
            "task": full_question,
...
def svamp_postprocess_answer(answer: Union[str, List[str]]) -> str:
    if isinstance(answer, list):
        for item in answer:
            parsed = svamp_postprocess_answer(item)
            if parsed:
                return parsed
        return ""
...
def svamp_answer_equal(predicted: Union[str, List[str]], target: Union[str, List[str]]) -> bool:
...
def svamp_get_predict(pred_str: Union[str, List[str]]) -> str:
    return svamp_postprocess_answer(pred_str)


def _parsed_or_first_raw(answer: Union[str, List[str]]) -> str:
    parsed = svamp_postprocess_answer(answer)
    if parsed:
        return parsed
    if isinstance(answer, list):
