...
def parse_args():
    parser = argparse.ArgumentParser(description="Run one MultiArith baseline method.")
...
def resolve_method_config(args) -> Dict[str, Any]:
    ...
    if mode == "MetaGPT":
        config.update({
            ...
            "notes": "No native implementation is available for MultiArith in this repository.",
        })
        return config
...
async def main():
    ...
    result_dir = Path(args.result_dir) if args.result_dir else Path(GDesigner_ROOT / "result" / "multiarith")
    ...
    dataset_eval = MultiArithDataset(
        args.eval_split,
